...
const Payment = ({ orderItems, total, user }) => {
    ...
    const id = user.length !== 0 ? user.user.id : null;
    const [name, setName] = useState(user.length !== 0 ? user.user.name : '');
    const [email, setEmail] = useState(user.length !== 0 ? user.user.email : '');
    const [phoneno, setPhoneno] = useState(user.length !== 0 ? user.user.phoneno : '');
    const [address, setAddress] = useState(user.length !== 0 ? user.user.address : '');
                    ...
                    <div className="col-sm-6 p-2">
                        ...
                        <div className="mb-3">
                            ...
                            <input type="text" className="form-control" id="address" placeholder="Enter user address"
                                name="address" defaultValue={user.length !== 0 ? user.user.address : ''} onChange={(e) => setAddress(e.target.value)} />
                        </div>
                        <div className="mb-3">
                            <label htmlFor="phoneno" className="form-label">Phone number:</label>
                            <input type="text" className="form-control" id="phoneno" placeholder="Enter user phoneno"
                                name="phoneno" defaultValue={user.length !== 0 ? user.user.phoneno : ''} onChange={(e) => setPhoneno(e.target.value)} />
                        </div>
